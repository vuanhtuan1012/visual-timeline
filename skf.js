...
function initialize(){
    ...
    navRows = 1;
    selFrame = document.getElementById("selFrame");
    isPlaying = false;
    keyframes = new Array();
    ...
    selFrame.style.top = video.offsetTop + video.height - selFrame.offsetHeight - 35 + 'px';
    selFrame.style.left = video.offsetLeft + Math.floor((video.width - selFrame.offsetWidth)/2) + 'px';
...
function captureFrame(){
    ...
    selFrame.disabled = true; // disable click on the button
    ...
    }else{ // do not have change in the number of rows
        iter = keyframes.length - 1;
        capture();
        video.controls = true; // enable control video
        selFrame.disabled = false; // enable click on the button
